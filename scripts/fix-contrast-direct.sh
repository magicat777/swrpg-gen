...
# Direct Container Accessibility Fixes
# This script applies contrast fixes directly to the running container

set -e

echo "🎯 Direct Container Accessibility Fixes"
echo "======================================="

# Step 1: Create the fixed theme file
echo "🔧 Creating accessibility-compliant factionThemes.ts..."

# Create a temporary file with our fixes
cat > /tmp/factionThemes_fixed.ts << 'EOF'
...
echo "✅ Fixed theme file created"

# Step 2: Copy to container
echo "📂 Copying fixed file to container..."
docker cp /tmp/factionThemes_fixed.ts swrpg-frontend:/app/src/styles/factionThemes.ts

# Step 3: Restart frontend container to ensure hot reload picks up changes
...
echo "🔍 Verifying accessibility fixes are active..."
echo "Empire theme text color:"
docker exec swrpg-frontend grep -A 1 "text: '#f7fafc'" /app/src/styles/factionThemes.ts

echo "Sith theme text color:"
docker exec swrpg-frontend grep -A 1 "text: '#ffffff'" /app/src/styles/factionThemes.ts

# Cleanup
rm /tmp/factionThemes_fixed.ts

echo ""
echo "🎉 DIRECT ACCESSIBILITY FIXES APPLIED!"
echo "🌐 Test at: http://localhost:3001/dashboard"
echo "🔧 Use WAVE extension to verify 5 contrast errors are now 0"